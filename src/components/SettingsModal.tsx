      ...
      >
        ...
        <div className="flex flex-1 min-h-0 flex-col sm:flex-row">
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'api' && (
              <div className="space-y-4">
                <div className="rounded-2xl bg-gray-50/80 p-4 border border-gray-200/60 dark:bg-white/[0.02] dark:border-white/[0.05] flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-100 mb-1">你的 API 信息，只留在本机</h4>
                    <p className="text-[13px] leading-relaxed text-gray-500 dark:text-gray-400">
                      API Key、连接配置与生成记录均保存在浏览器本地，不会上传至我们的服务器。密钥仅用于向你选择的服务商发起请求。
                    </p>
                  </div>
                </div>
              ...
              {apiProxyAvailable && activeProviderIsOpenAICompatible && (
                <div className="block">
                  ...
                  <div data-selectable-text className="text-xs text-gray-500 dark:text-gray-500">
                    {apiProxyLocked ? '当前部署已锁定 API 代理为开启，API URL 设置会被忽略。' : '当前部署提供同源代理时默认开启，可手动关闭。开启后用于解决浏览器跨域限制，API URL 设置会被忽略。自定义服务商请求会复用同一个部署端代理目标。'}
                  </div>
                </div>
              )}
            ...
            {activeTab === 'about' && (
              <div className="flex h-full min-h-[300px] flex-col items-center justify-center pb-8 px-6 text-center">
                <h4 className="text-[17px] font-bold text-gray-800 dark:text-gray-100">GPT Image Playground</h4>
                <p className="mt-2 text-sm font-mono text-gray-400 dark:text-gray-500">v{__APP_VERSION__}</p>
                <p className="mt-6 max-w-[360px] text-[13px] leading-relaxed text-gray-500 dark:text-gray-400">
                  基于开源项目二次开发，内网部署版本。
                </p>
                <a
                  href="https://github.com/ZMXJJ/gpt_image_playground"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-6 text-[13px] text-blue-500 hover:text-blue-600 dark:text-blue-400 dark:hover:text-blue-300 transition-colors"
                >
                  查看源码
                </a>
              </div>
